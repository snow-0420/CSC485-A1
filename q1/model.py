...
class ParserModel(nn.Module):
    ...
    def create_net_layers(self) -> None:
        ...
        """
        # *#* BEGIN YOUR CODE *#* #
        self.hidden_layer = nn.Linear((self.config.n_word_features +
                                      self.config.n_tag_features +
                                      self.config.n_deprel_features) *
                                      self.config.embed_size,
                                      self.config.hidden_size)

        self.output_layer = nn.Linear(self.config.hidden_size,
    ...
    def reshape_embedded(self, embedded_batch: torch.Tensor) -> torch.Tensor:
        ...
        """
        # *#* BEGIN YOUR CODE *#* #
        reshaped_batch = torch.reshape(embedded_batch,
                                       (embedded_batch.size(dim=0),
                                        embedded_batch.size(dim=1) *
                                        embedded_batch.size(dim=2)))
        # *** END YOUR CODE *** #
    ...
    def get_concat_embeddings(self, word_id_batch: torch.Tensor,
        ...
        """
        # *#* BEGIN YOUR CODE *#* #
        word_batch = self.word_embed(word_id_batch)
        tag_batch = self.tag_embed(tag_id_batch)
        deprel_batch = self.deprel_embed(deprel_id_batch)

        word_reshaped = self.reshape_embedded(word_batch)
        tag_reshaped = self.reshape_embedded(tag_batch)
        deprel_reshaped = self.reshape_embedded(deprel_batch)

        x = torch.cat((word_reshaped, tag_reshaped, deprel_reshaped), 1)
        # *** END YOUR CODE *** #
        return x
    ...
    def forward(self,
        ...
        # *#* BEGIN YOUR CODE *#* #
        h_pre_act = self.hidden_layer(x)
        h_act = F.relu(h_pre_act)
        h_drop = F.dropout(h_act, self.config.dropout, self.training)
