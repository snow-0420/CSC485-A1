...
class GraphDepModel(nn.Module):
    ...
    def create_arc_layers(self) -> None:
        ...
        * Initialize self.arc_W and self.arc_B according to a uniform
        ...
        """
        # *#* BEGIN YOUR CODE *#* #
        self.arc_h_mlp = nn.Sequential(nn.Linear(self.pt_width, cfg.ARC_DIM),
                                       nn.ReLU(),
        ...
        self.arc_d_mlp = nn.Sequential(nn.Linear(self.pt_width, cfg.ARC_DIM),
                                       nn.ReLU(),
                                       nn.Dropout(cfg.DROPOUT),
                                       nn.Linear(cfg.ARC_DIM, cfg.ARC_DIM),
                                       nn.ReLU(),
        ...
        # create the correct dimension tensor
        self.arc_W = nn.Parameter(torch.zeros(cfg.ARC_DIM, cfg.ARC_DIM))
        self.arc_B = nn.Parameter(torch.zeros(cfg.ARC_DIM))
        # fill the tensor with desired values
        nn.init.uniform_(self.arc_W, -sqrt(3 / cfg.ARC_DIM), sqrt(3 / cfg.ARC_DIM))
        nn.init.uniform_(self.arc_B, -sqrt(3 / cfg.ARC_DIM), sqrt(3 / cfg.ARC_DIM))
        # *** END YOUR CODE *** #
    ...
    def score_arcs(self, arc_head: Tensor, arc_dep: Tensor) -> Tensor:
        ...
        """
        # *#* BEGIN YOUR CODE *#* #
        arc_scores = torch.einsum('byi,ij,jxb->byx',
                                  arc_dep, self.arc_W,
                                  torch.transpose(arc_head, 0, 2)) + \
                     torch.unsqueeze(torch.einsum('bxj,j->bx', arc_head, self.arc_B), 2)
        # *** END YOUR CODE *** #
        return arc_scores
    ...
    def create_label_layers(self) -> None:
        ...
        * Initialize self.label_B to zeros.
        ...
        """
        # *#* BEGIN YOUR CODE *#* #
        self.label_h_mlp = nn.Sequential(nn.Linear(self.pt_width, cfg.LABEL_DIM),
                                         nn.ReLU(), nn.Dropout(cfg.DROPOUT),
                                         nn.Linear(cfg.LABEL_DIM, cfg.LABEL_DIM),
                                         nn.ReLU(), nn.Dropout(cfg.DROPOUT))

        self.label_d_mlp = nn.Sequential(nn.Linear(self.pt_width, cfg.LABEL_DIM),
                                         nn.ReLU(), nn.Dropout(cfg.DROPOUT),
                                         nn.Linear(cfg.LABEL_DIM, cfg.LABEL_DIM),
                                         nn.ReLU(), nn.Dropout(cfg.DROPOUT))

        # create tensors
        self.label_W = nn.Parameter(torch.zeros(cfg.LABEL_DIM, self._n_deprels, cfg.LABEL_DIM))
        self.label_h_W = nn.Parameter(torch.zeros(cfg.LABEL_DIM, self._n_deprels))
        self.label_d_W = nn.Parameter(torch.zeros(cfg.LABEL_DIM, self._n_deprels))
        self.label_B = nn.Parameter(torch.zeros(self._n_deprels))

        # fill tensors with desired values
        nn.init.uniform_(self.label_W, -sqrt(3 / cfg.LABEL_DIM), sqrt(3 / cfg.LABEL_DIM))
        nn.init.uniform_(self.label_h_W, -sqrt(3 / cfg.LABEL_DIM), sqrt(3 / cfg.LABEL_DIM))
        nn.init.uniform_(self.label_d_W, -sqrt(3 / cfg.LABEL_DIM), sqrt(3 / cfg.LABEL_DIM))
        # *** END YOUR CODE *** #
    ...
    def score_labels(self, label_head: Tensor, label_dep: Tensor) -> Tensor:
        ...
        """
        # *#* BEGIN YOUR CODE *#* #
        label_scores = torch.einsum('byi,irj,jxb->byxr',
                                    label_dep, self.label_W,
                                    torch.transpose(label_head, 0, 2)) + \
            torch.unsqueeze(torch.einsum('bxj,jr->bxr', label_head, self.label_h_W), 1) + \
            torch.unsqueeze(torch.einsum('byi,ir->byr', label_dep, self.label_d_W), 2) + \
            torch.unsqueeze(torch.unsqueeze(torch.unsqueeze(self.label_B, 0), 0), 0)
        # *** END YOUR CODE *** #
        return label_scores
    ...
    def mask_possible(self, shape: T.Sequence[int]):
        ...
        # *#* BEGIN YOUR CODE *#* #
        # root will not receive any dependency arc
        mask[:, 0, :, :] = 0
        # not root vertex will not give out any dependency relation 'root'
        mask[:, :, 1:, 0] = 0
        # self loop not allowed
        mask[:, torch.arange(shape[1]), torch.arange(shape[2]), :] = 0
        # *** END YOUR CODE *** #
        return mask
